$(document).ready(function () {
	...
	$('form').on('submit', function (e) {
		...
		var query = $.trim($('#local_input').val().toString());
		...
		if (query == '') {
			...
			script.src = 'https://api.worldweatheronline.com/free/v1/weather.ashx?q=' +
						 query + '&format=json&callback=getData&key=j8xvysb7t9jp2dvw7pwcbgs3';
			script.id = 'api_call'
...
var getData = function (data) {
	if ('error' in data.data) {
		...
		var temp_C = Math.round((temp - 32) * 5 / 9);

		$('#temp').text(temp_C + '℃');
		$('#desc').text(data.data.current_condition[0].weatherDesc[0].value);
		$('#local_input').val('').attr('placeholder', data.data.request[0].query);
